...
class PLMSSampler_trainer(object):
    ...
    def plms_sampling(self, cond, shape,
                      ...
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                      unconditional_guidance_scale=1., unconditional_conditioning=None,):
        device = self.quant_model.betas.device
        b = shape[0]
        if x_T is None:
            img = torch.randn(shape, device=device)
        else:
        ...
        def get_model_output(x, t):
            if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
                ...
                with torch.no_grad():
                    ...
                    t1 = t_in.detach().cpu()
                    c1 = c_in.detach().cpu()
                    e_t_uncond, e_t = self.model.apply_model(x1, t1, c1).chunk(2)
                e_t = e_t_uncond.cuda() + unconditional_guidance_scale * (e_t.cuda() - e_t_uncond.cuda())

                quant_e_t_uncond, quant_e_t = self.quant_model.apply_model(x_in.cuda(), t_in.cuda(), c_in.cuda()).chunk(2)
                quant_e_t = quant_e_t_uncond + unconditional_guidance_scale * (quant_e_t - quant_e_t_uncond)

            del (quant_e_t_uncond, e_t_uncond, x_in, t_in, c_in)
